...
//  SanFranSyms
//
//  Created by Vadim Zhuk on 23/10/2025.
//
...
final class SFSymbolSemanticSearchEngine {
    // MARK: - Configuration
    struct Config {
        var stopWords: Set<String> = []
        var lowercase: Bool = true
        var minTokenLength: Int = 1
        init() {}
    }
    ...
    func index(symbols: [SFSymbol], completion: (() -> Void)? = nil) {
    ...
    func search(_ query: String, topK: Int = 10) -> [SearchResult] {
    ...
    // Search in provided array
    func search(_ query: String, in symbols: [SFSymbol], topK: Int = 10) -> [SearchResult] {
        var results: [SearchResult] = []
        queue.sync {
    ...
    func upsert(symbol: SFSymbol, completion: (() -> Void)? = nil) {
        queue.async(flags: .barrier) {
    ...
    func remove(id: String, completion: (() -> Void)? = nil) {
        queue.async(flags: .barrier) {
            self.symbols[id] = nil
    ...
    func embeddingVector(for text: String) -> [Double]? {
        #if canImport(NaturalLanguage)
